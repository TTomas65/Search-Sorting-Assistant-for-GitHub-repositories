...
session_start(); // Csak egyszer indítjuk a session-t
...
try {
    ...
    // Létrehozunk egy "GitHub Stars" kategóriát minden importált repozitóriumhoz
    $stars_category_name = "GitHub Stars";
    $stars_category_id = null;
    
    // Ellenőrizzük, hogy létezik-e már ez a kategória
    $check_stars_sql = "SELECT category_id FROM user_categories WHERE user_id = $1 AND category_name = $2";
    $check_stars = pg_query_params($dbconn, $check_stars_sql, array($user_id, $stars_category_name));
    
    if ($check_stars === false) {
        addDebug("Error checking GitHub Stars category: " . pg_last_error($dbconn));
    } else if (pg_num_rows($check_stars) > 0) {
        // Ha már létezik a kategória, használjuk azt
        $stars_row = pg_fetch_assoc($check_stars);
        $stars_category_id = $stars_row['category_id'];
        addDebug("Using existing GitHub Stars category with ID: $stars_category_id");
    } else {
        // Létrehozzuk az új kategóriát
        $create_stars_sql = "INSERT INTO user_categories (user_id, category_name) VALUES ($1, $2) RETURNING category_id";
        $create_stars = pg_query_params($dbconn, $create_stars_sql, array($user_id, $stars_category_name));
        
        if ($create_stars === false) {
            addDebug("Error creating GitHub Stars category: " . pg_last_error($dbconn));
        } else {
            $stars_row = pg_fetch_assoc($create_stars);
            $stars_category_id = $stars_row['category_id'];
            addDebug("Created new GitHub Stars category with ID: $stars_category_id");
        }
    }
    
    // GitHub API hívás a csillagozott repozitóriumok lekérdezéséhez
    $page = 1;
    $per_page = 100;
    $all_starred_repos = [];
    $has_more = true;
    ...
    while ($has_more) {
        addDebug("Fetching starred repos page $page");
        $api_url = "https://api.github.com/user/starred?per_page=$per_page&page=$page";
        
        try {
            // Token validáció és API hívás a token manager használatával
            ...
            if ($http_code != 200) {
                addDebug("GitHub API error: HTTP code $http_code, Response: $response_data");
                throw new Exception("GitHub API error: HTTP code $http_code");
            }
        } catch (Exception $e) {
        ...
        $starred_repos = json_decode($response_data, true);
        
        if (empty($starred_repos) || !is_array($starred_repos)) {
            addDebug("No more starred repos or invalid response");
            $has_more = false;
            break;
        }
        
        $all_starred_repos = array_merge($all_starred_repos, $starred_repos);
        addDebug("Found " . count($starred_repos) . " starred repos on page $page");
        
        if (count($starred_repos) < $per_page) {
            $has_more = false;
        } else {
            $page++;
        }
    ...
    $total_repos = count($all_starred_repos);
    $response['total'] = $total_repos;
    addDebug("Total starred repos found: $total_repos");
    ...
    foreach ($all_starred_repos as $repo) {
        ...
        if ($check_favorite === false) {
            addDebug("Error checking favorite status for repo ID " . $repo['id'] . ": " . pg_last_error($dbconn));
            continue; // Ugrunk a következő repóra
        }
        
        if (pg_num_rows($check_favorite) > 0) {
            addDebug("Repository " . $repo['full_name'] . " is already a favorite, skipping...");
            $skipped++;
            continue;
        }
        
        // Minden repozitóriumot a GitHub Stars kategóriához rendeljük
        $repo_category_id = $stars_category_id !== null ? $stars_category_id : $category_id;
        addDebug("Assigning repository " . $repo['full_name'] . " to category ID: $repo_category_id");
        
        // Egyszerűsített megoldás: nem kérdezzük le a nyelveket, kontribútorokat és commit-okat
        // Ez csökkenti a hiba lehetőségét és gyorsítja a feldolgozást
    ...
    addDebug("Error in import_starred_repos.php: " . $e->getMessage());
    error_log('Error in import_starred_repos.php: ' . $e->getMessage());
